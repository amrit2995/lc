...
class FileUtilities:
    ...
    def file_exist(file_path=''):
        if os.path.isfile(file_path):
            logging.info(f"File: {file_path} exist")
            return True
        else:
            logging.info(f"File: {file_path} donot exist")
            return False
        
    @staticmethod
    def directory_exist(dir_path=''):
        if os.path.isdir(dir_path):
            logging.info(f"Directory {dir_path} exists")
            return True
        else:
            logging.info("Directory {dir_path} does not exist")
            return False

    @staticmethod
    def subprocess_response_decorator(func):
        """Specifically used when the script triggered by run_python_script().
            Use the decorator while returning the output from the triggered script.
        """
        ...
        def wrapper(*args, **kwargs):
            result = {}
            try:
                logging.info("Inside subprocess_response_decorator")
                output = func(*args, **kwargs)
                result["status"] = "success"
                result["output"] = output if output else "<No Output>"

            except SystemExit as e:
                logging.info("SystemExit the code. Probable reasons :- (necessary args not provided)")
                logging.info(e)
                result["status"] = "failed"
                result["output"] = f"SystemExit with code {e.code}"

            except Exception as e:
                logging.info("Hitting exception from response decorator")
                logging.error(f"{type(e).__name__} - {e}")
                result["status"] = "failed"
                result["output"] = f"{type(e).__name__} - {e}"
            finally:
                logging.info("Returning response from subprocess_response_decorator")

...
class GitCloneCommand:
    bash_command = ""
    base_git_repo_clone_command = "git -c \"http.extraHeader=Authorization: <bearer-auth-header>\" clone --single-branch --branch <git-repo-branch> \"<git-repo-url>\""
    ...
    def build(self):
        logging.info("Building git clone command")
        cmd_list_str = "\n".join(self.cmds_list)
        logging.info(f"Built git clone command complete.")
        return cmd_list_str
